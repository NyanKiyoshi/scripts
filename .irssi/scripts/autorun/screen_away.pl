...
# Register formats
Irssi::theme_register([ 'screen_away_crap', '{line_start}{hilight '.$IRSSI{'name'}.':} $0' ]);
...
if ($socket !~ /^No Sockets found/s) {
  ...
  if (length($socket_path) != length($socket)) {
    ...
    Irssi::printformat(MSGLEVEL_CLIENTCRAP, 'screen_away_crap', "error reading screen informations from:");
    Irssi::printformat(MSGLEVEL_CLIENTCRAP, 'screen_away_crap', "$socket");
    return;
  }
}
...
sub screen_away {
  ...
  if (Irssi::settings_get_bool($IRSSI{'name'} . '_active') == 1) {
    if ($away_status == 0) {
      ...
      Irssi::printformat(MSGLEVEL_CLIENTCRAP, 'screen_away_crap', "activating $IRSSI{'name'} (interval: " . Irssi::settings_get_int($IRSSI{'name'} . '_repeat') . " seconds)");
    }
    ...
    if ($away == 1 and $away_status != 1) {
      ...
      foreach $server (Irssi::servers()) {
        if (!$server->{usermode_away}) {
          # user isnt yet away
          $away{$server->{'tag'}} = 0;
          $server->command("AWAY " . (($server->{chat_type} ne 'SILC') ? "-one " : "") . "screen detached") if (!$server->{usermode_away});
        } else {
